...
if platform.system() == "Windows":
    # Avoid Qt trying to elevate DPI awareness after another component already set it.
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("MIMO / OPENAI API KEY", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._key_input = QLineEdit()
        self._key_input.setEchoMode(QLineEdit.EchoMode.Password)
        self._key_input.setPlaceholderText("AIza…")
        self._key_input.setPlaceholderText("sk-... or your MiMo key")
        ...
        layout.addWidget(_lbl("BASE URL", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._base_url_input = QLineEdit()
        self._base_url_input.setPlaceholderText("https://api.xiaomimimo.com/v1")
        self._base_url_input.setText("https://api.xiaomimimo.com/v1")
        self._base_url_input.setFont(QFont("Courier New", 10))
        self._base_url_input.setFixedHeight(32)
    ...
    def _submit(self):
        ...
        if not key or not base_url:
            ...
            self._base_url_input.setStyleSheet(
                self._base_url_input.styleSheet() +
                f" QLineEdit {{ border: 1px solid {C.RED}; }}"
            )
            return
        self.done.emit(key, base_url, self._sel_os)
...
class MainWindow(QMainWindow):
    ...
    def _build_right_panel(self) -> QWidget:
        ...
        self._voice_btn = QPushButton("VOICE TRIGGER")
        ...
        self._voice_btn.setStyleSheet(f"""
            QPushButton {{
                background: #0f1200; color: {C.ACC2};
                border: 1px solid {C.ACC2}; border-radius: 3px;
            }}
            QPushButton:hover {{ background: #1b1f00; }}
        """)
        self._voice_btn.clicked.connect(self._trigger_voice)
    ...
    def _trigger_voice(self):
        if self.on_voice_trigger:
            ...
            self._log.append_log("SYS: Voice trigger armed - speak now.")
    ...
    def _check_config(self) -> bool:
        ...
        try:
            ...
            api_key = d.get("llm_api_key") or d.get("gemini_api_key")
            return bool(api_key) and bool(d.get("llm_url")) and bool(d.get("os_system"))
        except Exception:
            return False
    ...
    def _on_setup_done(self, key: str, base_url: str, os_name: str):
...
class JarvisUI:
    ...
    def on_voice_trigger(self):
        return self._win.on_voice_trigger

    @on_voice_trigger.setter
    def on_voice_trigger(self, cb):
        self._win.on_voice_trigger = cb
